...
public class MedianOfTwoSortedArrays {
      public double findMidianSortedArrays_1(int A[], int B[]){
    	  int m = A.length, n =  B.length;
    	  int total = m+n, m1=0, m2=0, i=0, j=0;
    	  for (int k=1; k <= total/2+1;
    			  k++){
    		  int a = (i == m)? Integer.MAX_VALUE : A[i];
    		  int b = (j == n)? Integer.MAX_VALUE : A[j];
    		  m1=m2;
    		  m2 = Math.min(a,b);
    		  if (a>b) ++j;
    		  else ++i;
      ...
      public static void main(String[] args) {
  		...
  		double result = slt.findMidianSortedArrays_1(A, B);
