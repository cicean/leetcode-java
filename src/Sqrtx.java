...
public class Sqrtx {
	 ...
	 * 
	 * 
	 * 经过(xi, f(xi))这个点的切线方程为f(x) = f(xi) + f’(xi)(x -
	 * xi)，其中f'(x)为f(x)的导数，本题中为2x。令切线方程等于0，即可求出xi+1=xi - f(xi) / f'(xi)。
	 * 
	 * 继续化简，xi+1=xi - (xi2 - n) / (2xi) = xi - xi / 2 + n / (2xi) = xi / 2 + n /
	 * 2xi = (xi + n/xi) / 2。 牛顿迭代法也同样可以用于求解多次方程的解。 P.S.
	 * 本题是求解整数的平方根，并且返回值也是整型。在上述代码基础上稍微做修改， 就可以同样适用于double（仅限方法2）。
	 * 
	 * @param args
	 */
